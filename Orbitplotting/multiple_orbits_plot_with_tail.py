...
if comdenom == 'c2006p1':
    ...
    ax[0].plot(output[:,0,0],output[:,0,1],'k--')
    ax[0].plot(output[:tui,1,0],output[:tui,1,1],'k--')
    ax[0].scatter(0,0,c='orange',s=200)
    ax[0].text(com_vec[ecel,6]-8.5*d,com_vec[ecel,7]-d,"C/2006 P1",c='k', fontsize=fs)
    ax[0].set_xlim([-1.1,0.2])
    ax[0].set_ylim([-0.2,1.1])
    ax[0].set_title("XY plane (ecliptic)", fontsize = fs)
    ax[0].set_xlabel("X (AU)", fontsize = fs)
    ax[0].set_ylabel("Y (AU)", fontsize = fs)
    ...
    ax[1].plot(output[:,0,1],output[:,0,2],'k--')
    ax[1].plot(output[:tui,1,1],output[:tui,1,2],'k--')
    ax[1].scatter(0,0,c='orange',s=200)
    ax[1].set_xlim([-0.1,1.1])
    ax[1].set_ylim([-0.75,0.45])
    ax[1].set_title("YZ plane", fontsize = fs)
    ax[1].set_xlabel("Y (AU)", fontsize = fs)
    ax[1].set_ylabel("Z (AU)", fontsize = fs)
    ...
    ax[2].text(soho_earth_vec[ecel,6]+d,soho_earth_vec[ecel,7]+d,"SOHO",c='g', fontsize=fs)
    ax[2].text(stereob_earth_vec[ecel,6]+2*d,stereob_earth_vec[ecel,7],"STEREO-B",c='m', fontsize=fs)
    ax[2].text(stereoa_earth_vec[ecel,6]+d,stereoa_earth_vec[ecel,7]+d,"STEREO-B",c='r', fontsize=fs)
    ax[2].text(-0.005+d,-d,"Earth",c='b', fontsize=fs)
    ax[2].set_xlim([-0.006,0.018])
    ax[2].set_ylim([-0.018,0.006])
    ax[2].set_title("XY plane (ecliptic) relative to Earth", fontsize = fs)
    ax[2].set_xlabel("X (AU)", fontsize = fs)
    ax[2].set_ylabel("Y (AU)", fontsize = fs)
    ax[2].set_aspect('equal')
    ax[2].tick_params(axis='both', which='major', labelsize=ls)
    
    plt.subplots_adjust(left=0.011,
                    bottom=0.12, 
                    right=1.03, 
                    top=0.92, 
                    wspace=0.05, 
    ...
    plt.savefig('C:\PhD\Python\Python-Dust-Codes\Orbitplotting\plots\mcnaught_orbits_panel.png')
...
if comdenom == 'c2011l4':
    ...
    ax[0].plot(output[:tui,1,0],output[:tui,1,1],'k--')
    ax[0].scatter(0,0,c='orange',s=200)
    ax[0].text(stereoa_vec[ecel,6]-5*d,stereoa_vec[ecel,7]+2*d,"STEREO-A",c='r', fontsize=fs)
    ax[0].text(stereob_vec[ecel,6]-10*d,stereob_vec[ecel,7]+2*d,"STEREO-B",c='m', fontsize=fs)
    ax[0].text(earth_vec[ecel,6]+1.5*d,earth_vec[ecel,7]-2*d,"Earth",c='b', fontsize=fs)
    ax[0].text(earth_vec[ecel,6]+1.5*d,earth_vec[ecel,7]-2*d,"Earth",c='b', fontsize=fs)    
    ax[0].text(com_vec[ecel,6]-13*d,com_vec[ecel,7]+1*d,r"C/2011 L4",c='k', fontsize=fs)    
    ...
    ax[1].plot(stereob_vec[scel:ecel,7],stereob_vec[scel:ecel,8],c='m',lw=2)
    ax[1].scatter(stereob_vec[ecel,7],stereob_vec[ecel,8],c='m',marker='x',s=size)
    ax[1].plot(earth_vec[scel:ecel,7],earth_vec[scel:ecel,8],c='b',lw=2, zorder=1)
    ax[1].scatter(earth_vec[ecel,7],earth_vec[ecel,8],c='b',marker='x',s=size)
    ax[1].plot(com_vec[scel:ecel,7],com_vec[scel:ecel,8],c='k',lw=2)
    ax[1].scatter(com_vec[ecel,7],com_vec[ecel,8],c='k',marker='x',s=size)
    ax[1].plot(output[:,0,1],output[:,0,2],'k--')
    ax[1].plot(output[:tui,1,1],output[:tui,1,2],'k--')
    ax[1].scatter(0,0,c='orange',s=200, zorder=2)
    ...
    plt.savefig('C:\PhD\Python\Python-Dust-Codes\Orbitplotting\plots\panstarrs_orbits_panel.png')
...
if comdenom == 'c2002v1':
    ...
    ax[0].plot(com_vec[scel:ecel,6],com_vec[scel:ecel,7],c='k',lw=2)
    ax[0].scatter(com_vec[ecel,6],com_vec[ecel,7],c='k',marker='x',s=size)
    ax[0].text(soho_vec[ecel,6]+d,soho_vec[ecel,7]-d,"SOHO",c='g', fontsize=fs)
    ax[0].text(com_vec[ecel,6]-6*d,com_vec[ecel,7]-2.5*d,r"C/2002 V1",c='k', fontsize=fs)
    ax[0].plot(output[:,0,0],output[:,0,1],'k')
    ax[0].plot(output[:tui,1,0],output[:tui,1,1],'k')

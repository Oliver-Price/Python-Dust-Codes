...
for image_id in range(fits_total):
    ...
    diff = nuradecs - radec
    dtheta = np.linalg.norm(diff)/np.sqrt(2)
    diff[0,1] = diff[0,1]*np.cos(np.radians(radec[0,1]))
    dtheta1 = np.linalg.norm(diff)/np.sqrt(2)
    rat = np.linalg.norm(comveceq[imgcell,6:9] - obsveceq[imgcell,6:9])
    
    res[image_id] = np.pi*rat*dtheta/180*1.5e8
    res1[image_id] = np.pi*rat*dtheta1/180*1.5e8
...
plt.plot(tsj.jd,res)
plt.plot(tsj.jd,res1)
plt.plot(times.jd,r*(56/60/60*np.pi/180*1.5e8))
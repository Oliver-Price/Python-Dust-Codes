...
bads = r'C:\PhD\Comet_data\Comet for Deep Learning\Images\Colour\Orbitpathplots60+200\bad'

good = 0
bad = 0

if not os.path.exists(pltsav): os.makedirs(pltsav)

#dir_list = sorted(os.listdir(imgdir))
#fits_list = [(s.split('.')[0][:-4] + '.fits') for s in dir_list]

dir_list = sorted(os.listdir(bads))
fits_list = [(s.split('.')[0] + '.fits') for s in dir_list]
...
for fid in range(0,fits_total):
    ...
    try:
        ...
        if peaks.size == 1:
            plot_save = os.path.join(pltsav,"good2")
            if not os.path.exists(plot_save): os.makedirs(plot_save)
            good += 1
        else:
            plot_save = os.path.join(pltsav,"bad2")
            if not os.path.exists(plot_save): os.makedirs(plot_save)
            bad +=1
        
        plot_save = os.path.join(plot_save,filebase + '.png')

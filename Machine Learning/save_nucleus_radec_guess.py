...
for fid in range(0,fits_total):
    ...
    cdenom = comdenom
    try:
        comobs = orb_obs_new(cdenom, obsloc, pysav, orbitdir)
    except:
        cdenom = comdenom + '_1'
        comobs = orb_obs_new(cdenom, obsloc, pysav, orbitdir)
    ...
    #ras.append(cra[goodlocs][peaks[0]])
    #decs.append(comobs[goodlocs,6][peaks[0]])
    goods.append(goodlocs[peaks[0]])
    cdens.append(cdenom)
...
with open(denomsave, 'wb') as handle:
    pickle.dump(cdens, handle, protocol=pickle.HIGHEST_PROTOCOL)
  
with open(goodsave, 'wb') as handle:
    pickle.dump(goods, handle, protocol=pickle.HIGHEST_PROTOCOL)
'''    
with open(rasave, 'wb') as handle:
    pickle.dump(ras, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open(decsave, 'wb') as handle:
    pickle.dump(decs, handle, protocol=pickle.HIGHEST_PROTOCOL)    
'''
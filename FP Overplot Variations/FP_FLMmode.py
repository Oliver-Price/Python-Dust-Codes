...
while test_mode == True:
    ...
    if not os.path.exists(simsavefile ): os.makedirs(simsavefile)
    simsavefile  = os.path.join(simsavefile, filebase + '_simsetup_FLM.pickle')
    [t,bfrau,bfral,bno,h,cu,cl,cno,s0u,s0l,s0no,sc,test_mode] = simulation_setup_FLM(simsavefile)
    ...
    cvals = np.linspace(cl, cu, cno)
    
    s0vals = np.linspace(s0u,s0l,s0no)
    
    #create log distributed beta values, accounting that log(0) is impossible
    if (bfral == 0):
        bvals = np.logspace(np.log10(0.001), np.log10(bfrau), num=(bno-1))
        bvals = np.concatenate((np.array([0]), bvals))
        print ("Beta = 0, logarithmic spacing from Beta = 0.001")
    else:
        bvals = np.logspace(np.log10(bfral), np.log10(bfrau), num=(bno))
    ...
    simres = np.zeros((bno,cno,s0no,19),dtype = float)
    '''
    bmax = np.zeros((cno,s0no),dtype = int)
    bmin = np.zeros((cno,s0no),dtype = int)
    s0max = np.zeros((bno,cno),dtype = int)
    s0min = np.zeros((bno,cno),dtype = int)
    cmax = np.zeros((bno,s0no),dtype = int)
    cmin = np.zeros((bno,s0no),dtype = int)
    '''

#%%*********************************
#EIGHT CELL - Simulating Dust Motion
#***********************************
    simt10min = int(round(144*t))
    ...
    while (cidx < cno):
        ...
        while(s0idx < s0no):
            ...
            while (bidx >= 0 and (syn_exited_image == 0 or bidx >= bprev)):
                sim = FLM_sim(bvals[bidx],simt10min,50,1,pstart,efinp,dtmin,tswap,h,cvals[cidx],s0vals[s0idx],sc,mag=0,sign = -1)
                simres[bidx,cidx,s0idx,0] = float(simt10min)/144
                simres[bidx,cidx,s0idx,1] = bvals[bidx]
                simres[bidx,cidx,s0idx,2] = sim[0] #length of simulation in minutes
                simres[bidx,cidx,s0idx,3] = comcel-10*simt10min+sim[0] #find relevant cell for end of traj
                simres[bidx,cidx,s0idx,4:10] = sim[1] #finishing pos/vel
                simres[bidx,cidx,s0idx,10:12] = ecliptic_pos2radec(sim[1][0:3] - 
                obsveceq[int(simres[bidx,cidx,s0idx,3]),6:9],fixwrapsbool)
                point_in_image = int(box_Path.contains_point(
                (simres[bidx,cidx,s0idx,10],simres[bidx,cidx,s0idx,11])))
                simres[bidx,cidx,s0idx,12] = ra2xpix(simres[bidx,cidx,s0idx,10],border,pixwidth,rafmin,scale)
                simres[bidx,cidx,s0idx,13] = dec2ypix(simres[bidx,cidx,s0idx,11],border,pixheight,decmin,scale)                 
                simres[bidx,cidx,s0idx,14] = point_in_image
                syn_exited_image = syn_exit_tt[point_in_image,prev_stat]
                prev_stat = point_in_image
                simres[bidx,cidx,s0idx,17] = getdustphase(sim[1][0:3],obsveceq[int(simres[bidx,cidx,s0idx,3]),6:9])
                simres[bidx,cidx,s0idx,18] = sim[2]
                bidx -= 1
            s0idx += 1
            bprev = bidx
            try:simres[bidx+1-point_in_image,cidx,s0idx,15] = 0
            except:pass
            try:
                bmin[cidx,s0idx] = simres[:,cidx,s0idx,14].nonzero()[0][0]
                bmax[cidx,s0idx] = simres[:,cidx,s0idx,14].nonzero()[0][-1]
            except:pass
        cidx += 1
        print (float(cidx)*100/cno)
    ...
    for bidx in range(0,bno):
        [minblock,maxblock] = find_largest_nonzero_block(simres[bidx,:,s0idx,14])
        if minblock == None:
...
#*****************************
    ...
    for ca in range(cno):
        for sa in range(s0no):
            for ba in range(bno-1):
                d.line([(simres[ba,ca,sa,12],simres[ba,ca,sa,13]), \
                        (simres[ba+1,ca,sa,12],simres[ba+1,ca,sa,13])],\
                        fill = chrfill,width = 1)
    
    fontloc = r'C:\Windows\winsxs\amd64_microsoft-windows-f..etype-lucidaconsole_31bf3856ad364e35_6.1.7600.16385_none_5b3be3e0926bd543\lucon.ttf'
    fnt = ImageFont.truetype(fontloc, 20)    
    
    cimgdir = os.path.join(imagedir, 'FLMplots')
    if not os.path.exists(cimgdir): os.makedirs(cimgdir)
    cimgsav = os.path.join(cimgdir, 'FP_' + filebase + str(h) + '_'
                     + str(cu) + '_' + str(cl)+ '_' + str(s0u) + '_'
                     + str(s0l) + '_' + str(sc) + '_.png')    

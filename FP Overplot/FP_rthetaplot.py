...
obsOrbPla = earthplane2orbplane(x_ang,z_ang,obsveceq)
comOrbPla = earthplane2orbplane(x_ang,z_ang,comveceq)
...
while test_mode == True:
    ...
    vis_box_path = np.zeros((5,2),dtype=float)
    vis_box_path[0,0] = rafmax; vis_box_path[0,1] = decmax
    vis_box_path[1,0] = rafmin; vis_box_path[1,1] = decmax
    vis_box_path[2,0] = rafmin; vis_box_path[2,1] = decmin
    vis_box_path[3,0] = rafmax; vis_box_path[3,1] = decmin
    vis_box_path[4,0] = rafmax; vis_box_path[4,1] = decmax
    
    vis_Path = mplPath.Path(vis_box_path)
    ...
    if com_in_image == True: image_case = 1
        ...
        for tval_test in range(0, tno):	
                ...
                simt1min = int(round(1440*tvals[tval_test]))
                pstart = comveceq[comcel-simt1min,6:12]
                sim_lo = part_sim_fine(bvals[0],simt1min,30,3,pstart,efinp)
                sim_hi = part_sim_fine(bvals[bno-1],simt1min,30,3,pstart,efinp)
                ra_dec_lo_test = pos2radec(sim_lo[1][0:3] -
                obsveceq[int(comcel-simt1min+sim_lo[0]),6:9],fixwrapsbool)
                ra_dec_hi_test = pos2radec(sim_hi[1][0:3] -
                obsveceq[int(comcel-simt1min+sim_hi[0]),6:9],fixwrapsbool)
                ...
                sync_intersects[tval_test] = vis_Path.intersects_path(sync_Path)
    ...
    cmetime = astropy.time.Time(datetime.datetime(cmeYear, cmeMon, cmeDay,
    						                            cmeHour , cmeMin, cmeSec))
    
    cmedelta = (ctime - cmetime).jd
...
#***********************************
    ...
    if image_case == 1:
        ...
        while (tidx < tno):
            ...
            pstart = comOrbPla[comcel-simt1min]
            while (bidx < bno and point_in_vis == 1):
                sim = part_sim_CME(bvals[bidx],simt1min,250,1,pstart,efinp,cmedelta)
                ...
                simres[tidx,bidx,4:10] = orbplane2earthplane(-x_ang,-z_ang,sim[1])#finishing pos/vel
                simres[tidx,bidx,10:12] = pos2radec(simres[tidx,bidx,4:7] - 
                ...
                point_in_vis = int(vis_Path.contains_point(
                (simres[tidx,bidx,10],simres[tidx,bidx,11])))
                ...
                simres[tidx,bidx,17] = point_in_vis
                bidx += 1
            try:simres[tidx,bidx - 1 + point_in_vis,15] = 0
            except: pass
            try:bmax[tidx] = simres[tidx,:,17].nonzero()[0][-1]
            except: bmax[tidx] = 0            
            tidx += 1
            print (float(tidx)*100/tno)
            
        simres[np.where(simres[:,:,17]==0)] = 0
        bidx_list = np.arange(bno)
        deletions = 0
        for bidx in range(0,bno):
            [minblock,maxblock] = find_largest_nonzero_block(simres[:,bidx,17])
            if minblock == None:
        ...
        tidx_list = np.unique(np.nonzero(simres[:,:,17])[0])
    ...
    if image_case == 2:
        ...
        for tidx_list_val in range(0,np.size(tidx_list)):
            ...
            syn_exited_image = 0
            prev_stat = 0
            simt1min = round(1440*tvals[tidx])
            pstart = comOrbPla[comcel-simt1min]
            while (bidx >= 0 and (syn_exited_image == 0 or bidx >= bprev)):
                sim = part_sim_CME(bvals[bidx],simt1min,250,3,pstart,efinp,cmedelta)
                simres[tidx,bidx,0] = float(simt1min)/144
                simres[tidx,bidx,1] = bvals[bidx]
                simres[tidx,bidx,2] = sim[0] #length of simulation in minutes
                simres[tidx,bidx,3] = comcel-simt1min+sim[0] #find relevant cell for end of traj
                simres[tidx,bidx,4:10] = orbplane2earthplane(-x_ang,-z_ang,sim[1]) #finishing pos/vel
                ...
                simres[tidx,bidx,13] = dec2ypix(simres[tidx,bidx,11],border,pixheight,decmin,scale)                 
                simres[tidx,bidx,14] = point_in_image
                syn_exited_image = syn_exit_tt[point_in_vis,prev_stat]
                prev_stat = point_in_vis
                simres[tidx,bidx,17] = point_in_vis
                bidx -= 1
            print (float(tidx)*100/tno)
            bprev = bidx
            ...
            try:
                bmin[tidx] = simres[tidx,:,17].nonzero()[0][0]
                bmax[tidx] = simres[tidx,:,17].nonzero()[0][-1]
    ...
    non_zeros_0 = simres[:,:,17].nonzero()[0]
    non_zeros_1 = simres[:,:,17].nonzero()[1]
    no_points = non_zeros_0.size
    ...
    if (sav_bool == True):
        simressavefile = os.path.join(pysav, 'simres_CME')
...
#*****************************
    ...
    if "Data Points" in drawopts: draw_datap_CME(drfill,d,simres)
